...
export class RepartidoresService {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!infoPrev) {
          ...
          this.db.object(`rateRepartidor/resumen/${idNegocio}`).update(calificacion);
        } else {
  ...
  eliminarRepartidor(repartidor: Repartidor) {
    ...
    this.db.object(`rateRepartidor/${idNegocio}/detalles/${repartidor.preview.id}`).remove();
    this.db.object(`rateRepartidor/${idNegocio}/resumen/${repartidor.preview.id}`).remove();
  }
