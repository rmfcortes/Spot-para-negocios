...
export class PasilloService {
  ...
  async setPalabrasPasillos(pasillos: Pasillo[]) {
    ...
    const x = claves.toLocaleLowerCase().split(' ').filter((item, i, allItems) => i === allItems.indexOf(item));
    console.log(x);
    // .join(' ').toLocaleLowerCase();
    // claves = Array.from(new Set(claves.split(' '))).toString().toLocaleLowerCase().replace(/,/gi, ' ');
  ...
  async setPalabrasPasillo(pasillo: string) {
    ...
    claves = claves.split(' ').filter((item, i, allItems) => i === allItems.indexOf(item)).join(' ').toLocaleLowerCase();
    // claves = Array.from(new Set(claves.split(' '))).toString().toLocaleLowerCase().replace(/,/gi, ' ');
    this.palabrasService.updateClaves(claves);
  }
