...
export class ProductosService {
  ...
  setProducto(producto: Producto, categoria: string, complementos: Complemento[], tipo: string) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await this.db.object(`perfiles/${idNegocio}/${tipo}`).query.ref.transaction(productos => productos ? productos + 1 : 1);
        resolve();
  ...
  deleteProducto(producto: Producto, tipo: string, categoria: string, prodsLength: number) {
    const region = this.uidService.getRegion();
    return new Promise(async (resolve, reject) => {
      const idNegocio = this.uidService.getUid();
      ...
      await this.db.object(`perfiles/${idNegocio}/${tipo}`).query.ref.transaction(productos => productos ? productos - 1 : 0);
      resolve();
      if (prodsLength === 1) {
        ...
        await this.db.object(`isOpen/${region}/${idNegocio}`).remove();
  ...
  uploadFoto(foto: string, producto: Producto, origen: string): Promise<any> {
    return new Promise (async (resolve, reject) => {
      ...
      const ref = this.fireStorage.ref(`negocios/productos/${idNegocio}/${producto.id}`);
