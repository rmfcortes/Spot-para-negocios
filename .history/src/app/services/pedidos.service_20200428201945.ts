...
export class PedidosService {
  ...
  listenRepartidor() {
    ...
    this.db.object(`pedidos/activos/${uid}/repartidor_pendiente`).query.ref.on('child_removed', snapshot => {
      ...
      console.log(id);
      this.pedidos_pendientes_repartidor = this.pedidos_pendientes_repartidor.filter(p => p.pedido.id !== id)
    })
  }

  listenRepartidorTs(idPedido: string) {
    const uid = this.uidService.getUid()
    return this.db.object(`pedidos/activos/${uid}/detalles/${idPedido}/repartidor`).query.ref
  }
