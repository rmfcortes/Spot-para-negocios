...
export class HomePage {
  ...
  ionViewWillLeave() {
    ...
    this.pedidoService.getPedidos().query.ref.off('child_changed');
  ...
  getPedido(pedido: Pedido, i: number) {
    ...
    this.iSel = i;
    if (this.pedido.aceptado && !this.pedido.repartidor) this.listenRepartidoresPendientes()
  }
  ...
  listenRepartidoresPendientes() {
    console.log('Escucha repartidor pendiente');
    this.pedidoService.getPedidos().query.ref.on('child_changed', snapshot => {
      this.ngZone.run(() => {
        ...
        if (pedido.repartidor) {
          ...
          this.pedidos[i].repartidor = pedido.repartidor
          let repartidores_pendientes = false
          for (const pedido of this.pedidos) {
            if (!pedido.repartidor) repartidores_pendientes = true
          }
          if (!repartidores_pendientes) this.pedidoService.getPedidos().query.ref.off('child_changed')
        }
      });
    });
  }
