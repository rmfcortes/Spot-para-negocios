...
export class ProductosPage implements OnInit {
  ...
  // Funciones escritorio
    ...
    // Info inicio
    async newEditProd(producto: Producto, i: number, y: number) {
      if (!producto) {
        const agregados: number = await this.productoService.getProductosAgregados();
        const cuenta = this.uidService.getCuenta();
        let permitidos;
    ...
    getListaPasillos() {
      this.pasillosService.getPasillos(this.categoria).then((pasillos) => {
        ...
        if (this.listaPasillos.length === 0) {
  ...
  getPasillosLista() {
    this.pasillosService.getPasillos(this.categoria).then((pasillos: Pasillo[]) => {
      this.listaPasillos = pasillos;
    });
  }
  ...
  editPasillo(i, name) {
    this.alertService.presentAlertPrompt(`Edita ${name}`, 'Ej. Botanas, Bebidas')
      .then((nombre: string) => {
        ...
        this.listaPasillos[i].nombre = nombre;
        this.pasillosService.editPasillo(this.categoria, i, name, nombre);
      });
  }
