  ...
  entregaPlaneada() {
    ...
    .then((resp: any) => {
      ...
      if (num) {
        ...
        const dia = this.datePipe.transform(this.pedido.aceptado, 'EEEE d, MMMM, y').toString()
        console.log(dia);
        this.alertService.presentAlertAction('Entrega', `Confirma si tendrás listos los productos el 
        ${{dia}}`)
