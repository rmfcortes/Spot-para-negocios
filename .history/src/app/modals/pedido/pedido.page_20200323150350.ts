  ...
  entregaPlaneada() {
    ...
    .then(async (resp: any) => {
      ...
      if (num) {
        ...
        let dia = await this.datePipe.transform(this.pedido.aceptado, 'EEEE d, MMMM, y').toString()
        const day = new Date(this.pedido.aceptado).toLocaleDateString()
        console.log(day);
        dia = dia.toString()
        console.log(dia);
        this.alertService.presentAlertAction('Entrega', `Confirma si tendrás listos los productos el ${{day}}`)
