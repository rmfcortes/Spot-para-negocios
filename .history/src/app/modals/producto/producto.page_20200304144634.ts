  ...
  eliminarProducto() {
    if (this.length > 1) {
      this.alertService.presentAlertAction(`Eliminar ${this.producto.nombre}`,
        ...
        .then(async (resp) => {
          if (resp) {
            this.productoService.deleteProducto(this.producto, this.tipo, this.categoria, this.length);
            this.alertService.presentToast('Artículo eliminado');
            this.modalCtrl.dismiss(true);
          }
        });
    } else {
      this.alertService.presentAlertAction(`Último producto en tu lista`,
      'Si eliminas el último producto, además de la información perteneciente al mismo, perderás de forma permanente '+
      'información importante como Palabras Clave de búsqueda, y ya no aparecerás en la lista de negocios de Spot')
      .then(async (resp) => {
        if (resp) {
          this.productoService.deleteProducto(this.producto, this.tipo, this.categoria, this.length);
          this.alertService.presentToast('Artículo eliminado');
