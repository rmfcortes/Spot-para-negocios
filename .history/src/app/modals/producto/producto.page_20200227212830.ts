...
export class ProductoPage implements OnInit {
  ...
  async guardarCambios() {
    this.guardando = true;
    try {
      const region = await this.productoService.getRegion();
      if (!region) {
        this.alertService.presentAlert('Completa tu perfil', 'No pudimos encontrar la región a la que perteneces. ' +
        'Por favor revisa tu perfil y complétalo de ser necesario');
      ...
      if (this.pasilloViejo && this.pasilloViejo !== this.producto.pasillo) {
        this.productoService.changePasillo(this.categoria, this.pasilloViejo, this.producto.id, region);
      }
      await this.productoService.setProducto(this.producto, this.categoria, this.complementos, region);
      this.guardando = false;
  ...
  eliminarProducto() {
    this.alertService.presentAlertAction(`Eliminar ${this.producto.nombre}`,
      ...
      .then(async (resp) => {
        if (resp) {
          const region = await this.productoService.getRegion();
          this.productoService.deleteProducto(this.producto, this.tipo, this.categoria, region);
          this.modalCtrl.dismiss(true);
        }
      });
  }
